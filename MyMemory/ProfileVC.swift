...
class ProfileVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    let tableview = UITableView() // 프로필 목록
...
//        let image = UIImage(named: "account.jpg")
        ...
        self.drawBtn()
        // 탭 제스처 등록하고, profile(_:) 액션시트 창 메소드와 연결
...
//            cell.detailTextLabel?.text = "박힘찬"
            cell.detailTextLabel?.text = self.uinfo.name ?? "Login please"
        case 1:
            cell.textLabel?.text = "계정"
//            cell.detailTextLabel?.text = "sbpigeon@naver.com"
    ...
    @objc func doLogin(_ sender: Any) {
        ...
        loginAlert.addAction(UIAlertAction(title: "Login", style: .destructive) { (_) in
            ...
            if self.uinfo.login(account: account, passwd: passwd) {
                ...
                self.tableview.reloadData() // 테이블 뷰를 갱신한다.
    ...
    @objc func doLogout(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "확인", style: .destructive) { (_) in
            if self.uinfo.logout() {
                // TODO: (로그아웃 처리할 내용이 여기에 들어갈 예정입니다.)
                self.tableview.reloadData() // 테이블 뷰를 갱신한다.
                self.profileImage.image = self.uinfo.profile // 이미지 프로필을 갱신한다.
                self.drawBtn() // 로그인 상태에 따라 적절히 로그인/로그아웃 버튼을 출력한다.
            }
    ...
    func drawBtn() {
        ...
        btn.center.y = view.frame.size.height / 2
