...
class SideBarVC: UITableViewController {
    ...
    // 아이콘 데이터 배열
    let icons = [UIImage(named: "icon01"), UIImage(named: "icon02"), UIImage(named: "icon03"), UIImage(named: "icon04"),
                 UIImage(named: "icon05"), UIImage(named: "icon06")]
    override func viewDidLoad() {
        ...
        self.nameLabel.frame = CGRect(x: 70, y: 15, width: 100, height: 30) // 위치 크기
//        self.nameLabel.text = "박힘찬" // 기본 텍스트
        ...
        self.emailLabel.frame = CGRect(x: 70, y: 30, width: 150, height: 30) // 위치 크기
//        self.emailLabel.text = "sbpigeon@naver.com" // 기본 텍스트
        self.emailLabel.textColor = .white // 텍스트 색상
        self.emailLabel.font = UIFont.systemFont(ofSize: 11) // 폰트 사이즈
        self.emailLabel.backgroundColor = .clear // 배경색상
        headerView.addSubview(self.emailLabel)
        // 프로필 이미지를 등록하고, 헤더 뷰에 추가
//        let defaultProfile = UIImage(named: "account.jpg")
//        self.profileImage.image = defaultProfile // 이미지 등록
        self.profileImage.frame = CGRect(x: 10, y: 10, width: 50, height: 50) // 위치 크기
        // 프로필 이미지 둥글게 만들기
